...
def get_convert_fname(fname, extension, directory, convert_directory):
    return fname.replace('tiff', extension).replace(directory, 
                                                    convert_directory)
...
@click.command()
@click.option('--directory', default='/data1/visionlab/data/Messidor_2_original/IMAGES', show_default=True,
              help="Directory with original images.")
@click.option('--convert_directory', default='/data1/visionlab/data/Messidor_2_original/IMAGES_Teamo_O_512', show_default=True,
...
def main(directory, convert_directory, test, crop_size, extension):
    ...
    filenames = [os.path.join(dp, f) for dp, dn, fn in os.walk(directory)
                 for f in fn if f.endswith('JPG') or f.endswith('png')] 
