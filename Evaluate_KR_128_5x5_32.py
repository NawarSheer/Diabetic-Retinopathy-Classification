...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 12:33:10 2020
...
model = load_model('./Experiments/PH_1/EX_44/EX_44.hdf5', custom_objects={'RMSPooling2D': RMSPooling2D,'Lambda':Lambda,'tf':tf})
model.summary()

test_df = pd.read_csv('./labels/EyePACS_2015_new_test.csv')

#val_sp = np.random.rand(len(test_df)) < 0.16
#val_df = test_df[val_sp]
...
for i, name in tqdm(enumerate(test_df['image'])):
    ...
    image = image[...,::-1]
    y_test = model.predict((image[np.newaxis]/255))
